...
class LayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-12):
        ...
        self.quantize_fn = weight_quantize_fn(bitW=args.bitW)
        self.act_q = activation_quantize_fn(abitW=args.abitW)
    ...
    def forward(self, x):
        ...
        return self.quantize_fn(self.weight) * self.act_q(x) + self.bias
...
class Attention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        query = self.act_q(query)
        key = self.act_q(key)
        value = self.act_q(value)
