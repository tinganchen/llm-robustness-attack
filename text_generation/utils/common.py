...
class checkpoint():
    ...
    def save_model(self, state, epoch, is_best):
        save_path = f'{self.ckpt_dir}/model_best.pt' # f'{self.ckpt_dir}/model_{epoch}.pt'
...
def hr(logit, label, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        label = label[:, -1]
        last_token_logits = logit[:, -2, :]
        #pred = last_token_logits.argmax(dim=-1)
        
        _, pred = last_token_logits.topk(maxk, 1, True, True)
        pred = pred.t()
